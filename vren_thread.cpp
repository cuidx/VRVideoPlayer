...
int vren_thread::InitRender_gl(size_t width, size_t height)
{
	...
	if (m_viewMode == VM_VR)
	{
		gluPerspective(50, (GLfloat)width / height, 0.1f, 100.0f);    //设置投影矩阵
	}
	else
	{
...
void vren_thread::ResizeViewport(size_t width, size_t height) {
	...
	else
	{
		...
		gluPerspective(60, (GLfloat)width / height, 0.1f, 100.0f);    //设置投影矩阵
		glMatrixMode(GL_MODELVIEW);    //设置矩阵模式为视图矩阵(模型)
		glLoadIdentity();    //变为单位矩阵
	}
}

void vren_thread::SetViewMode(int wm)
{
	m_viewMode = wm;
	RECT rcClient;
	::GetClientRect(m_hwhd, &rcClient);
	TryResizeViewport(rcClient.right - rcClient.left, rcClient.bottom - rcClient.top);
}
...
void vren_thread::KeyboardMsg(int key)
{
	switch (key)
	{
